...
const page = () => {
  ...
  useEffect(() => {
    dispatch(fetchDetails({ filterType: "movie", movieId: "1241982" }));
    dispatch(fetchSimilarVideos({ filterType: "movie", movieId: "1241982" }));
  }, []);

  const Details = useSelector(
    (state: any) => state?.provider?.movieDetails?.details
  ) || {
    results: [],
  };
  const Credits = useSelector(
    (state: any) => state?.provider?.movieDetails?.credits
  ) || {
    results: [],
  };
  ...
  return (
    <div className="pt-20 text-white px-[10%]">
      <section className="flex w-full py-12">
        <div className="w-[65%] flex rounded-xl px-12">
          <img
        ...
        <div className="w-full">
          ...
          <div className="flex items-center gap-4">
            <div>
              <CircularProgressBar
                percentage={Details?.vote_average * 10 || 0}
              />
            </div>
          ...
          {Credits?.cast?.map((cast: any) => (
            <div key={cast.id} className="flex flex-col justify-center">
              ...
              <div className="text-center mt-4">
                <h6 className="font-semibold text-[0.9rem]">{cast?.name}</h6>
                <p className="text-gray-400 text-[0.8rem]">{cast?.character}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="flex items-start gap-8 w-full overflow-x-scroll my-4">
          {videos?.results.map((vid: any) => (
            ...
            >
              ...
              <p className="text-white text-[1rem] my-2 flex">{vid?.name}</p>
            </div>
          ))}
        </div>
